...
//  s[0] = new CustomSoftwareSerial(10, 9);
//  s[0]->begin(9600, CSERIAL_8E1);
  load_software_serial_settings();

}

int get_serial_param_bits_from_conf(unsigned char conf){
  //configs
    //data,parity,stop
...
int get_serial_param_baud_from_conf(unsigned char setting){
  int temp = ((setting & 0b11100000)>>5);
  if(temp<7) return BAUD_RATE[temp];
  return 9600;//default
}
...
void load_software_serial_settings(){
  ...
  for(unsigned char i =0; i<8; i++){
    setting = eeprom_read(i); //for each serial i, setting
    
          //baud there are only 7 baud rates, thus 0-6 baud indexs, 7 to say unset   
    if((setting & 0b11100000)!= 0b11100000){ //0b11100000 is for unconfigured
        
        
        apply_setting_software_serial(
          i, //serial port
          get_serial_param_baud_from_conf(setting), //baud_rate
          get_serial_param_bits_from_conf(setting & 0b00011111) //conf
        );
...
void apply_setting_software_serial(unsigned char serial, int baud_rate, int conf){
   s[serial]->begin(baud_rate, conf);         // Baud rate: 9600, configuration: CSERIAL_8N1
   //s[serial]->begin(9600, CSERIAL_8E1);
}
...
void change_state(){
  ...
  unsigned char serial = (cmd >> 5) & 0x07; // serial port, specifies which serial port.
  unsigned char mode = (cmd & 0b00011111); // mode

    
  if(mode == M_CONF_WRITE){ // configuration write mode
    
    unsigned char conf = read_usart(); //read configurations to be written

    //extract baud_rate, conf
    // baud [7:5] conf[4:0]
    apply_setting_software_serial(
          serial, //serial port
          get_serial_param_baud_from_conf(conf), //baud_rate
          get_serial_param_bits_from_conf(conf & 0b00011111) //conf
    );
    
    //change serial port setting, save and setting in the eeprom
    eeprom_write(serial, conf); //write to memory
    ...
    write_usart(data); //working fine both the data and serial_port

    
  }else if(mode == M_DATA_READ){ //data read mode
    // read data from serial using software serial
    char resp = read_s_serial(serial);
    // now send that
...
void test1(){
  // buffer ic pin test
  for(int i=0; i<8; i++){
    pinMode(RX[i], INPUT); pinMode(TX[i], OUTPUT);

    digitalWrite(TX[i], HIGH); // writing TXn to HIGH and checked using multimeter. TXn IC working.
    //digitalWrite(TX[i], LOW); // writing TXn to LOW and checked using multimeter. TXn IC working.
  }
}

/** ---------------------------------------------------------------------------------------------test 2 : Custom Software Serial -- passed --------------- **/
CustomSoftwareSerial* customSerial;
void test2(){
  customSerial = new CustomSoftwareSerial(10, 9); // rx, tx ????
  customSerial->begin(9600, CSERIAL_8E1);         // Baud rate: 9600, configuration: CSERIAL_8N1
  
  char msg[] = {'h','e','l','l','o'}; //6 chars
  for(int i=0; i<5;i++) customSerial->write(msg[i]);
}

/** ---------------------------------------------------------------------------------------------test 3 : Custom Software Serial -- passed--------------- **/

void test3(){
  s[0] = new CustomSoftwareSerial(10, 9); // rx, tx ????
  s[0]->begin(9600, CSERIAL_8E1);         // Baud rate: 9600, configuration: CSERIAL_8N1
  
  char msg[] = {'h','e','l','l','o'}; //6 chars
  for(int i=0; i<5;i++) s[0]->write(msg[i]);
}

/** ---------------------------------------------------------------------------------------------test 4 : Custom Software Serial -- passed--------------- **/

void test4(){
  s[1] = new CustomSoftwareSerial(11, 8); // rx, tx ????
  s[1]->begin(9600, CSERIAL_8E1);         // Baud rate: 9600, configuration: CSERIAL_8N1
  
  char msg[] = {'h','e','l','l','o'}; //6 chars
  for(int i=0; i<5;i++) s[1]->write(msg[i]);
}

/** ---------------------------------------------------------------------------------------------test 5 : send data using configured serial - passed --------------- **/

void test5(){
//  s[0] = new CustomSoftwareSerial(11, 8); // rx, tx ????
//  s[0]->begin(9600, CSERIAL_8E1);         // Baud rate: 9600, configuration: CSERIAL_8N1
//  
  char msg[] = {'h','e','l','l','o'}; //6 chars
  for(int i=0; i<5;i++) s[0]->write(msg[i]);
